...
public class StepValidationManager {
    ...
    public void validateUpToStep(TemporaryPropertyStepSaveRequestDTO dto) {
        TemporaryPropertyStepStatus currentStep = dto.getStepStatus();
        List<TemporaryPropertyStepStatus> stepsToValidate = TemporaryPropertyStepStatus.getStepsUpTo(currentStep);

        //이게 정말 효율 적인 방법일까..
        for (TemporaryPropertyStepStatus step : stepsToValidate) {
            StepValidationKey key = new StepValidationKey(dto.getStepStatus(), dto.getKind());
